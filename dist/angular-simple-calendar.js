angular.module('500tech.simple-calendar', []).directive('simpleCalendar', function () {
    return {
        ...
        controller: ['$scope', function ($scope) {
            ...
            $scope.onEventClick = function (events) {
                $scope.options.eventClick(events)
            }
            ...
            bindEvent = function (date) {
                ...
                $scope.events.forEach(function (event) {
                    ...
                    if (date.year === event.date.getFullYear() && date.month === event.date.getMonth() && date.day === event.date.getDate()) {
                        ...
                        // if there are date classes, add them
                        if (event.dateClass) {
                            date.dateClass = date.dateClass + ' ' + event.dateClass;
                            date.dateClass = date.dateClass.trim();
                        }
                    }
                });
            };
